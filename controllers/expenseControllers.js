...
module.exports.get_today_expense = async (req, res) => {
  const id = req.user._id;
  var startOfToday = new Date('2023-09-13');
  try {
    let expenses = await Expense.find({ id ,date:startOfToday}).sort({ date: -1 });

    console.log("expense        sssssssssssssssssssssssssssssssss"+expenses)
    let filterData = [];
...
module.exports.get_singledate_expense = async (req, res) => {
  const id = req.user._id;
  const sel_d = req.params.date;
  sel_date = new Date(sel_d);
  console.log(id)

console.log(sel_date)
  try {
    let expenses = await Expense.find({ id ,date:new Date(sel_d)});
